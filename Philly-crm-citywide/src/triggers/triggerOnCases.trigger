            ...
            if (!CaseFieldUpdate.inFutureContext) {
                ...
                for (Case c : Trigger.new) {
                    ...
                    if(ownerName != null && ownerName == '311 Mobile Requests' && c.Validated_Cases_from_PS__c == FALSE)
                        ...
                        if((c.Status == 'Closed' && c.OwnerId==oldcase.OwnerId )|| c.Service_Request_Type__c  != oldCase.Service_Request_Type__c )
                            CaseId.add(c.id);
                        ...
                        if(ownerName != null && ownerName == 'CitiWorks Reject' && c.Centerline_2272x__c != null && c.Centerline_2272x__c != 0 && c.Centerline_2272x__c != 0.0)
                            CaseId.add(c.id);  
                        else if((c.Status == 'Closed' && c.OwnerId==oldcase.OwnerId )|| c.Service_Request_Type__c  != oldCase.Service_Request_Type__c )
                            CaseId.add(c.id);       
